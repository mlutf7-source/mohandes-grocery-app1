        ...
        <div className="relative mb-4">
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              />
              {search && (
                <button onClick={() => { setSearch(''); setShow(false); }} className="absolute left-3 top-1/2 -translate-y-1/2">✕</button>
              )}
              {show && (
                <>
            ...
            <div key={p.id}>
              <Card className="mb-3">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => startEdit(p)} className="flex items-center gap-1 px-3 py-1.5 bg-info/10 text-info rounded-btn text-small font-semibold"><Edit2 size={14} />تعديل</button>
                    <button onClick={() => delPur(p.id)} className="flex items-center gap-1 px-3 py-1.5 bg-danger/10 text-danger rounded-btn text-small font-semibold"><Trash2 size={14} />حذف</button>
                  </div>
                </div>
                ...
                <div className="space-y-1 mb-3">
                  {p.items.map((i: any, idx: number) => (
                    <div key={idx} className="flex items-center text-small">
                      <span className="flex-[3] font-semibold">{i.productName}{i.unit === 'كرتون' && i.boxQty ? ` (كرتون ${i.boxQty} ح)` : ''}</span>
                ...
                <div className="flex gap-2 mt-3">
                  <button onClick={() => printElement(`invoice-${p.id}`)} className="flex items-center gap-1 px-3 py-1.5 bg-primary-light text-primary rounded-btn text-small font-semibold"><Printer size={14} />طباعة</button>
                  <button onClick={() => sharePdfFromElement(`invoice-${p.id}`, `فاتورة شراء #${p.invoiceNo || p.id.slice(-6)}`)} className="flex items-center gap-1 px-3 py-1.5 bg-primary-light text-primary rounded-btn text-small font-semibold"><Share2 size={14} />مشاركة</button>
                </div>
              </Card>
