...
export default function PurchasesReport() {
  ...
  const reportId = 'purchases-report-print';

  return (
    <div className="page-container">
      <h1 className="page-title">تقرير المشتريات</h1>
      ...
      <div id={reportId} className="bg-white text-black p-4 space-y-4" style={{ fontFamily: 'Cairo, sans-serif', direction: 'rtl' }}>
        <div className="text-center mb-4">
          ...
          <h3 className="text-md font-bold">تقرير المشتريات</h3>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <Card className="!bg-info/5 text-center"><p className="text-small">الإجمالي</p><p className="text-financial text-info">{fmt(totalPurchases)}</p></Card>
          ...
          {purchases.map((p: any) => (
            <Card key={p.id} className="!bg-white !border !border-gray-200 !shadow-none sale-card" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
              <div className="flex justify-between items-center text-sm pb-1.5">
                <span className="font-bold">فاتورة شراء #{p.invoiceNo || p.id.slice(-6)}</span>
                <span className="text-small text-text-secondary">{dt(p.createdAt)}</span>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-2 space-y-1 text-sm">
                ...
                {p.remaining > 0 && <div className="flex justify-between font-bold text-warning"><span>المتبقي</span><span>{fmt(p.remaining)} {currency}</span></div>}
              </div>
            </Card>
          ))}
        </div>
      ...
      <div className="flex gap-2 mt-4 no-print">
        ...
        <Button fullWidth variant="secondary" onClick={() => sharePdfFromElement(reportId, 'تقرير المشتريات')}><Share2 size={16} />مشاركة PDF</Button>
      </div>
    </div>
  );
                      }
